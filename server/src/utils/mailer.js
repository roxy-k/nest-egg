...
const buildTransport = () => {
  ...
  try {
    ...
    const socketTimeout = Number.parseInt(
      process.env.SMTP_SOCKET_TIMEOUT_MS || connectionTimeout || "10000",
      10,
    );
    ...
    cachedTransport = nodemailer.createTransport({
      ...
      socketTimeout: Number.isNaN(socketTimeout) ? undefined : socketTimeout,
    });
    cachedState = cachedTransport ? "ready" : "missing";
  } catch (err) {
...
export const sendPasswordResetEmail = async ({
  ...
  if (!token || !email || !to) {
    return { ok: false, skipped: true, reason: "missing_parameters" };
  }
  ...
  if (!transport) {
    ...
    return { ok: false, skipped: true, reason: "transport_unavailable" };
  }
  ...
  if (!from) {
    ...
    return { ok: false, skipped: true, reason: "from_missing" };
  }
  ...
  try {
    const timeoutMs = Number.parseInt(
      process.env.SMTP_SEND_TIMEOUT_MS || process.env.SMTP_SOCKET_TIMEOUT_MS || "15000",
      10,
    );
    ...
    if (!Number.isNaN(timeoutMs) && timeoutMs > 0) {
      ...
      if (!timeoutResult.ok && timeoutResult.reason === "timeout") {
        console.error(
          `Password reset email timed out after ${timeoutMs} ms. Check SMTP connectivity.`,
        );
      }
